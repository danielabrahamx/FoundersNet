/* eslint-disable */
/**
 * This file was automatically generated by @algorandfoundation/algokit-client-generator.
 * DO NOT MODIFY IT BY HAND.
 * requires: @algorandfoundation/algokit-utils: ^7
 */
import { type AlgorandClient } from '@algorandfoundation/algokit-utils/types/algorand-client'
import { ABIReturn, AppReturn, SendAppTransactionResult } from '@algorandfoundation/algokit-utils/types/app'
import { Arc56Contract, getArc56ReturnValue, getABIStructFromABITuple } from '@algorandfoundation/algokit-utils/types/app-arc56'
import {
  AppClient as _AppClient,
  AppClientMethodCallParams,
  AppClientParams,
  AppClientBareCallParams,
  CallOnComplete,
  AppClientCompilationParams,
  ResolveAppClientByCreatorAndName,
  ResolveAppClientByNetwork,
  CloneAppClientParams,
} from '@algorandfoundation/algokit-utils/types/app-client'
import { AppFactory as _AppFactory, AppFactoryAppClientParams, AppFactoryResolveAppClientByCreatorAndNameParams, AppFactoryDeployParams, AppFactoryParams, CreateSchema } from '@algorandfoundation/algokit-utils/types/app-factory'
import { TransactionComposer, AppCallMethodCall, AppMethodCallTransactionArgument, SimulateOptions, RawSimulateOptions, SkipSignaturesSimulateOptions } from '@algorandfoundation/algokit-utils/types/composer'
import { SendParams, SendSingleTransactionResult, SendAtomicTransactionComposerResults } from '@algorandfoundation/algokit-utils/types/transaction'
import { Address, encodeAddress, modelsv2, OnApplicationComplete, Transaction, TransactionSigner } from 'algosdk'
import SimulateResponse = modelsv2.SimulateResponse

export const APP_SPEC: Arc56Contract = {"name":"PredictionMarket","structs":{"BetStruct":[{"name":"betId","type":"uint64"},{"name":"eventId","type":"uint64"},{"name":"bettor","type":"address"},{"name":"outcome","type":"bool"},{"name":"amount","type":"uint64"},{"name":"claimed","type":"bool"}],"EventStruct":[{"name":"eventId","type":"uint64"},{"name":"name","type":"string"},{"name":"endTime","type":"uint64"},{"name":"resolved","type":"bool"},{"name":"outcome","type":"bool"},{"name":"totalYesBets","type":"uint64"},{"name":"totalNoBets","type":"uint64"},{"name":"totalYesAmount","type":"uint64"},{"name":"totalNoAmount","type":"uint64"}]},"methods":[{"name":"create_application","args":[{"type":"address","name":"admin","desc":"The account that will have admin privileges"}],"returns":{"type":"void"},"actions":{"create":["NoOp"],"call":[]},"readonly":false,"desc":"Initialize the application\nSets the admin address and initializes counters","events":[],"recommendations":{}},{"name":"create_event","args":[{"type":"string","name":"name"},{"type":"uint64","name":"end_time"}],"returns":{"type":"uint64"},"actions":{"create":[],"call":["NoOp"]},"readonly":false,"desc":"Create a new prediction event (admin only)\nArgs:     name: Name/description of the event     end_time: Unix timestamp when betting ends      Returns:     The event ID of the newly created event      Raises:     Error if caller is not admin     Error if end_time is not in the future","events":[],"recommendations":{}},{"name":"place_bet","args":[{"type":"uint64","name":"event_id"},{"type":"bool","name":"outcome"},{"type":"pay","name":"payment"}],"returns":{"type":"void"},"actions":{"create":[],"call":["NoOp"]},"readonly":false,"desc":"Place a bet on an event\nThis method expects to be called as part of an atomic transaction group where the first transaction is a payment to the application account.\nArgs:     event_id: ID of the event to bet on     outcome: True for YES, False for NO     payment: Payment transaction with the bet amount      Raises:     Error if event doesn't exist     Error if event is already resolved     Error if betting period has ended     Error if payment amount is 0     Error if payment receiver is not the application account","events":[],"recommendations":{}},{"name":"resolve_event","args":[{"type":"uint64","name":"event_id"},{"type":"bool","name":"outcome"}],"returns":{"type":"void"},"actions":{"create":[],"call":["NoOp"]},"readonly":false,"desc":"Resolve an event with the final outcome (admin only)\nAdmin can resolve early if needed (no endTime check)\nArgs:     event_id: ID of the event to resolve     outcome: True for YES, False for NO      Raises:     Error if caller is not admin     Error if event doesn't exist     Error if event is already resolved","events":[],"recommendations":{}},{"name":"claim_winnings","args":[{"type":"uint64","name":"bet_id"}],"returns":{"type":"void"},"actions":{"create":[],"call":["NoOp"]},"readonly":false,"desc":"Claim winnings for a winning bet\nSends payout from app account to the bettor via inner transaction\nArgs:     bet_id: ID of the bet to claim      Raises:     Error if bet doesn't exist     Error if caller is not the bettor     Error if winnings already claimed     Error if event not resolved     Error if bet did not win","events":[],"recommendations":{}},{"name":"get_user_bets","args":[{"type":"address","name":"user","desc":"Address of the user"}],"returns":{"type":"(uint64,uint64,address,bool,uint64,bool)[]","desc":"Array of Bet structs for the user"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get all bets for a user","events":[],"recommendations":{}},{"name":"get_total_bets","args":[{"type":"uint64","name":"event_id"}],"returns":{"type":"(uint64,uint64)"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get total bet counts for an event\nArgs:     event_id: ID of the event      Returns:     Tuple of (yes_bets, no_bets)      Raises:     Error if event doesn't exist","events":[],"recommendations":{}},{"name":"get_event","args":[{"type":"uint64","name":"event_id"}],"returns":{"type":"(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)","struct":"EventStruct"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get event details\nArgs:     event_id: ID of the event      Returns:     Event struct with all event data      Raises:     Error if event doesn't exist","events":[],"recommendations":{}},{"name":"get_all_events","args":[],"returns":{"type":"(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]","desc":"Array of all Event structs"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get all events","events":[],"recommendations":{}},{"name":"get_contract_balance","args":[],"returns":{"type":"uint64","desc":"Current balance of the application account in microAlgos"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get contract/app account balance","events":[],"recommendations":{}},{"name":"emergency_withdraw","args":[],"returns":{"type":"void"},"actions":{"create":[],"call":["NoOp"]},"readonly":false,"desc":"Emergency withdraw function for admin\nTransfers entire app balance to admin account\nRaises:     Error if caller is not admin","events":[],"recommendations":{}},{"name":"get_admin","args":[],"returns":{"type":"address","desc":"Address of the admin account"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get the admin address","events":[],"recommendations":{}},{"name":"get_event_counter","args":[],"returns":{"type":"uint64","desc":"Number of events created"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get the current event counter","events":[],"recommendations":{}},{"name":"get_bet_counter","args":[],"returns":{"type":"uint64","desc":"Number of bets placed"},"actions":{"create":[],"call":["NoOp"]},"readonly":true,"desc":"Get the current bet counter","events":[],"recommendations":{}}],"arcs":[22,28],"desc":"\n    Algorand-based prediction market smart contract\n    \n    Features:\n    - Admin can create prediction events\n    - Users can place bets (YES/NO) with ALGO\n    - Admin can resolve events (with early resolution support)\n    - Winners can claim proportional payouts\n    ","networks":{},"state":{"schema":{"global":{"ints":2,"bytes":1},"local":{"ints":0,"bytes":0}},"keys":{"global":{"admin":{"keyType":"AVMString","valueType":"address","key":"YWRtaW4="},"event_counter":{"keyType":"AVMString","valueType":"AVMUint64","key":"ZXZlbnRfY291bnRlcg=="},"bet_counter":{"keyType":"AVMString","valueType":"AVMUint64","key":"YmV0X2NvdW50ZXI="}},"local":{},"box":{}},"maps":{"global":{},"local":{},"box":{"events":{"keyType":"uint64","valueType":"EventStruct","prefix":"ZXZlbnRz"},"bets":{"keyType":"uint64","valueType":"BetStruct","prefix":"YmV0cw=="},"user_bets":{"keyType":"address","valueType":"uint64[]","prefix":"dXNlcl9iZXRz"},"event_bets":{"keyType":"uint64","valueType":"uint64[]","prefix":"ZXZlbnRfYmV0cw=="}}}},"bareActions":{"create":[],"call":[]},"sourceInfo":{"approval":{"sourceInfo":[{"pc":[470],"errorMessage":"Bet amount must be greater than 0"},{"pc":[894],"errorMessage":"Bet did not win"},{"pc":[806],"errorMessage":"Bet does not exist"},{"pc":[278],"errorMessage":"End time must be in the future"},{"pc":[748],"errorMessage":"Event already resolved"},{"pc":[462],"errorMessage":"Event betting period has ended"},{"pc":[422,723,1128,1164],"errorMessage":"Event does not exist"},{"pc":[452],"errorMessage":"Event is already resolved"},{"pc":[865],"errorMessage":"Event not resolved yet"},{"pc":[1394],"errorMessage":"No balance to withdraw"},{"pc":[826],"errorMessage":"Not the bet owner"},{"pc":[103],"errorMessage":"OnCompletion must be NoOp"},{"pc":[272],"errorMessage":"Only admin can create events"},{"pc":[1384],"errorMessage":"Only admin can emergency withdraw"},{"pc":[714],"errorMessage":"Only admin can resolve events"},{"pc":[477],"errorMessage":"Payment must be to app account"},{"pc":[841],"errorMessage":"Winnings already claimed"},{"pc":[1365,1389],"errorMessage":"account funded"},{"pc":[270,712,1379,1435],"errorMessage":"check self.admin exists"},{"pc":[481,804,1066,1456],"errorMessage":"check self.bet_counter exists"},{"pc":[817,1077],"errorMessage":"check self.bets entry exists"},{"pc":[562],"errorMessage":"check self.event_bets entry exists"},{"pc":[282,420,721,1126,1162,1188,1445],"errorMessage":"check self.event_counter exists"},{"pc":[435,730,1169,1205],"errorMessage":"check self.events entry exists"},{"pc":[534],"errorMessage":"check self.user_bets entry exists"},{"pc":[255],"errorMessage":"invalid number of bytes for (len+utf8[])"},{"pc":[403,705],"errorMessage":"invalid number of bytes for bool8"},{"pc":[263,392,697,798,1120,1156],"errorMessage":"invalid number of bytes for uint64"},{"pc":[230,1015],"errorMessage":"invalid number of bytes for uint8[32]"},{"pc":[538,566,1083],"errorMessage":"max array length exceeded"},{"pc":[413],"errorMessage":"transaction type is pay"}],"pcOffsetMethod":"none"},"clear":{"sourceInfo":[],"pcOffsetMethod":"none"}},"source":{"approval":"","clear":"I3ByYWdtYSB2ZXJzaW9uIDExCiNwcmFnbWEgdHlwZXRyYWNrIGZhbHNlCgovLyBhbGdvcHkuYXJjNC5BUkM0Q29udHJhY3QuY2xlYXJfc3RhdGVfcHJvZ3JhbSgpIC0+IHVpbnQ2NDoKbWFpbjoKICAgIHB1c2hpbnQgMSAvLyAxCiAgICByZXR1cm4K"},"byteCode":{"approval":"","clear":"C4EBQw=="},"compilerInfo":{"compiler":"puya","compilerVersion":{"major":5,"minor":2,"patch":0}},"events":[],"templateVariables":{}} as unknown as Arc56Contract

/**
 * A state record containing binary data
 */
export interface BinaryState {
  /**
   * Gets the state value as a Uint8Array
   */
  asByteArray(): Uint8Array | undefined
  /**
   * Gets the state value as a string
   */
  asString(): string | undefined
}

class BinaryStateValue implements BinaryState {
  constructor(private value: Uint8Array | undefined) {}

  asByteArray(): Uint8Array | undefined {
    return this.value
  }

  asString(): string | undefined {
    return this.value !== undefined ? Buffer.from(this.value).toString('utf-8') : undefined
  }
}

/**
 * Expands types for IntelliSense so they are more human readable
 * See https://stackoverflow.com/a/69288824
 */
export type Expand<T> = T extends (...args: infer A) => infer R
  ? (...args: Expand<A>) => Expand<R>
  : T extends infer O
    ? { [K in keyof O]: O[K] }
    : never


// Type definitions for ARC-56 structs

export type BetStruct = {
  betId: bigint,
  eventId: bigint,
  bettor: string,
  outcome: boolean,
  amount: bigint,
  claimed: boolean
}


/**
 * Converts the ABI tuple representation of a BetStruct to the struct representation
 */
export function BetStructFromTuple(abiTuple: [bigint, bigint, string, boolean, bigint, boolean]) {
  return getABIStructFromABITuple(abiTuple, APP_SPEC.structs.BetStruct, APP_SPEC.structs) as BetStruct
}

export type EventStruct = {
  eventId: bigint,
  name: string,
  endTime: bigint,
  resolved: boolean,
  outcome: boolean,
  totalYesBets: bigint,
  totalNoBets: bigint,
  totalYesAmount: bigint,
  totalNoAmount: bigint
}


/**
 * Converts the ABI tuple representation of a EventStruct to the struct representation
 */
export function EventStructFromTuple(abiTuple: [bigint, string, bigint, boolean, boolean, bigint, bigint, bigint, bigint]) {
  return getABIStructFromABITuple(abiTuple, APP_SPEC.structs.EventStruct, APP_SPEC.structs) as EventStruct
}

/**
 * The argument types for the PredictionMarket contract
 */
export type PredictionMarketArgs = {
  /**
   * The object representation of the arguments for each method
   */
  obj: {
    'create_application(address)void': {
      /**
       * The account that will have admin privileges
       */
      admin: string
    }
    'create_event(string,uint64)uint64': {
      name: string
      endTime: bigint | number
    }
    'place_bet(uint64,bool,pay)void': {
      eventId: bigint | number
      outcome: boolean
      payment: AppMethodCallTransactionArgument
    }
    'resolve_event(uint64,bool)void': {
      eventId: bigint | number
      outcome: boolean
    }
    'claim_winnings(uint64)void': {
      betId: bigint | number
    }
    'get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]': {
      /**
       * Address of the user
       */
      user: string
    }
    'get_total_bets(uint64)(uint64,uint64)': {
      eventId: bigint | number
    }
    'get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)': {
      eventId: bigint | number
    }
    'get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]': Record<string, never>
    'get_contract_balance()uint64': Record<string, never>
    'emergency_withdraw()void': Record<string, never>
    'get_admin()address': Record<string, never>
    'get_event_counter()uint64': Record<string, never>
    'get_bet_counter()uint64': Record<string, never>
  }
  /**
   * The tuple representation of the arguments for each method
   */
  tuple: {
    'create_application(address)void': [admin: string]
    'create_event(string,uint64)uint64': [name: string, endTime: bigint | number]
    'place_bet(uint64,bool,pay)void': [eventId: bigint | number, outcome: boolean, payment: AppMethodCallTransactionArgument]
    'resolve_event(uint64,bool)void': [eventId: bigint | number, outcome: boolean]
    'claim_winnings(uint64)void': [betId: bigint | number]
    'get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]': [user: string]
    'get_total_bets(uint64)(uint64,uint64)': [eventId: bigint | number]
    'get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)': [eventId: bigint | number]
    'get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]': []
    'get_contract_balance()uint64': []
    'emergency_withdraw()void': []
    'get_admin()address': []
    'get_event_counter()uint64': []
    'get_bet_counter()uint64': []
  }
}

/**
 * The return type for each method
 */
export type PredictionMarketReturns = {
  'create_application(address)void': void
  'create_event(string,uint64)uint64': bigint
  'place_bet(uint64,bool,pay)void': void
  'resolve_event(uint64,bool)void': void
  'claim_winnings(uint64)void': void
  'get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]': [bigint, bigint, string, boolean, bigint, boolean][]
  'get_total_bets(uint64)(uint64,uint64)': [bigint, bigint]
  'get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)': EventStruct
  'get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]': [bigint, string, bigint, boolean, boolean, bigint, bigint, bigint, bigint][]
  'get_contract_balance()uint64': bigint
  'emergency_withdraw()void': void
  'get_admin()address': string
  'get_event_counter()uint64': bigint
  'get_bet_counter()uint64': bigint
}

/**
 * Defines the types of available calls and state of the PredictionMarket smart contract.
 */
export type PredictionMarketTypes = {
  /**
   * Maps method signatures / names to their argument and return types.
   */
  methods:
    & Record<'create_application(address)void' | 'create_application', {
      argsObj: PredictionMarketArgs['obj']['create_application(address)void']
      argsTuple: PredictionMarketArgs['tuple']['create_application(address)void']
      returns: PredictionMarketReturns['create_application(address)void']
    }>
    & Record<'create_event(string,uint64)uint64' | 'create_event', {
      argsObj: PredictionMarketArgs['obj']['create_event(string,uint64)uint64']
      argsTuple: PredictionMarketArgs['tuple']['create_event(string,uint64)uint64']
      returns: PredictionMarketReturns['create_event(string,uint64)uint64']
    }>
    & Record<'place_bet(uint64,bool,pay)void' | 'place_bet', {
      argsObj: PredictionMarketArgs['obj']['place_bet(uint64,bool,pay)void']
      argsTuple: PredictionMarketArgs['tuple']['place_bet(uint64,bool,pay)void']
      returns: PredictionMarketReturns['place_bet(uint64,bool,pay)void']
    }>
    & Record<'resolve_event(uint64,bool)void' | 'resolve_event', {
      argsObj: PredictionMarketArgs['obj']['resolve_event(uint64,bool)void']
      argsTuple: PredictionMarketArgs['tuple']['resolve_event(uint64,bool)void']
      returns: PredictionMarketReturns['resolve_event(uint64,bool)void']
    }>
    & Record<'claim_winnings(uint64)void' | 'claim_winnings', {
      argsObj: PredictionMarketArgs['obj']['claim_winnings(uint64)void']
      argsTuple: PredictionMarketArgs['tuple']['claim_winnings(uint64)void']
      returns: PredictionMarketReturns['claim_winnings(uint64)void']
    }>
    & Record<'get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]' | 'get_user_bets', {
      argsObj: PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']
      argsTuple: PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']
      /**
       * Array of Bet structs for the user
       */
      returns: PredictionMarketReturns['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']
    }>
    & Record<'get_total_bets(uint64)(uint64,uint64)' | 'get_total_bets', {
      argsObj: PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)']
      argsTuple: PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']
      returns: PredictionMarketReturns['get_total_bets(uint64)(uint64,uint64)']
    }>
    & Record<'get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)' | 'get_event', {
      argsObj: PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']
      argsTuple: PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']
      returns: PredictionMarketReturns['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']
    }>
    & Record<'get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]' | 'get_all_events', {
      argsObj: PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']
      argsTuple: PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']
      /**
       * Array of all Event structs
       */
      returns: PredictionMarketReturns['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']
    }>
    & Record<'get_contract_balance()uint64' | 'get_contract_balance', {
      argsObj: PredictionMarketArgs['obj']['get_contract_balance()uint64']
      argsTuple: PredictionMarketArgs['tuple']['get_contract_balance()uint64']
      /**
       * Current balance of the application account in microAlgos
       */
      returns: PredictionMarketReturns['get_contract_balance()uint64']
    }>
    & Record<'emergency_withdraw()void' | 'emergency_withdraw', {
      argsObj: PredictionMarketArgs['obj']['emergency_withdraw()void']
      argsTuple: PredictionMarketArgs['tuple']['emergency_withdraw()void']
      returns: PredictionMarketReturns['emergency_withdraw()void']
    }>
    & Record<'get_admin()address' | 'get_admin', {
      argsObj: PredictionMarketArgs['obj']['get_admin()address']
      argsTuple: PredictionMarketArgs['tuple']['get_admin()address']
      /**
       * Address of the admin account
       */
      returns: PredictionMarketReturns['get_admin()address']
    }>
    & Record<'get_event_counter()uint64' | 'get_event_counter', {
      argsObj: PredictionMarketArgs['obj']['get_event_counter()uint64']
      argsTuple: PredictionMarketArgs['tuple']['get_event_counter()uint64']
      /**
       * Number of events created
       */
      returns: PredictionMarketReturns['get_event_counter()uint64']
    }>
    & Record<'get_bet_counter()uint64' | 'get_bet_counter', {
      argsObj: PredictionMarketArgs['obj']['get_bet_counter()uint64']
      argsTuple: PredictionMarketArgs['tuple']['get_bet_counter()uint64']
      /**
       * Number of bets placed
       */
      returns: PredictionMarketReturns['get_bet_counter()uint64']
    }>
  /**
   * Defines the shape of the state of the application.
   */
  state: {
    global: {
      keys: {
        admin: string
        eventCounter: bigint
        betCounter: bigint
      }
      maps: {}
    }
    box: {
      keys: {}
      maps: {
        events: Map<bigint | number, EventStruct>
        bets: Map<bigint | number, BetStruct>
        userBets: Map<string, bigint[]>
        eventBets: Map<bigint | number, bigint[]>
      }
    }
  }
}

/**
 * Defines the possible abi call signatures.
 */
export type PredictionMarketSignatures = keyof PredictionMarketTypes['methods']
/**
 * Defines the possible abi call signatures for methods that return a non-void value.
 */
export type PredictionMarketNonVoidMethodSignatures = keyof PredictionMarketTypes['methods'] extends infer T ? T extends keyof PredictionMarketTypes['methods'] ? MethodReturn<T> extends void ? never : T  : never : never
/**
 * Defines an object containing all relevant parameters for a single call to the contract.
 */
export type CallParams<TArgs> = Expand<
  Omit<AppClientMethodCallParams, 'method' | 'args' | 'onComplete'> &
    {
      /** The args for the ABI method call, either as an ordered array or an object */
      args: Expand<TArgs>
    }
>
/**
 * Maps a method signature from the PredictionMarket smart contract to the method's arguments in either tuple or struct form
 */
export type MethodArgs<TSignature extends PredictionMarketSignatures> = PredictionMarketTypes['methods'][TSignature]['argsObj' | 'argsTuple']
/**
 * Maps a method signature from the PredictionMarket smart contract to the method's return type
 */
export type MethodReturn<TSignature extends PredictionMarketSignatures> = PredictionMarketTypes['methods'][TSignature]['returns']

/**
 * Defines the shape of the keyed global state of the application.
 */
export type GlobalKeysState = PredictionMarketTypes['state']['global']['keys']

/**
 * Defines the shape of the keyed box state of the application.
 */
export type BoxKeysState = PredictionMarketTypes['state']['box']['keys']


/**
 * Defines supported create method params for this smart contract
 */
export type PredictionMarketCreateCallParams =
  | Expand<CallParams<PredictionMarketArgs['obj']['create_application(address)void'] | PredictionMarketArgs['tuple']['create_application(address)void']> & {method: 'create_application'} & {onComplete?: OnApplicationComplete.NoOpOC} & CreateSchema>
  | Expand<CallParams<PredictionMarketArgs['obj']['create_application(address)void'] | PredictionMarketArgs['tuple']['create_application(address)void']> & {method: 'create_application(address)void'} & {onComplete?: OnApplicationComplete.NoOpOC} & CreateSchema>
/**
 * Defines arguments required for the deploy method.
 */
export type PredictionMarketDeployParams = Expand<Omit<AppFactoryDeployParams, 'createParams' | 'updateParams' | 'deleteParams'> & {
  /**
   * Create transaction parameters to use if a create needs to be issued as part of deployment; use `method` to define ABI call (if available) or leave out for a bare call (if available)
   */
  createParams?: PredictionMarketCreateCallParams
}>


/**
 * Exposes methods for constructing `AppClient` params objects for ABI calls to the PredictionMarket smart contract
 */
export abstract class PredictionMarketParamsFactory {
  /**
   * Gets available create ABI call param factories
   */
  static get create() {
    return {
      _resolveByMethod<TParams extends PredictionMarketCreateCallParams & {method: string}>(params: TParams) {
        switch(params.method) {
          case 'create_application':
          case 'create_application(address)void':
            return PredictionMarketParamsFactory.create.createApplication(params)
        }
        throw new Error(`Unknown ' + verb + ' method`)
      },

      /**
       * Constructs create ABI call params for the PredictionMarket smart contract using the create_application(address)void ABI method
       *
       * @param params Parameters for the call
       * @returns An `AppClientMethodCallParams` object for the call
       */
      createApplication(params: CallParams<PredictionMarketArgs['obj']['create_application(address)void'] | PredictionMarketArgs['tuple']['create_application(address)void']> & AppClientCompilationParams & {onComplete?: OnApplicationComplete.NoOpOC}): AppClientMethodCallParams & AppClientCompilationParams & {onComplete?: OnApplicationComplete.NoOpOC} {
        return {
          ...params,
          method: 'create_application(address)void' as const,
          args: Array.isArray(params.args) ? params.args : [params.args.admin],
        }
      },
    }
  }

  /**
   * Constructs a no op call for the create_event(string,uint64)uint64 ABI method
   *
  * Create a new prediction event (admin only)
  Args:     name: Name/description of the event     end_time: Unix timestamp when betting ends      Returns:     The event ID of the newly created event      Raises:     Error if caller is not admin     Error if end_time is not in the future

   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static createEvent(params: CallParams<PredictionMarketArgs['obj']['create_event(string,uint64)uint64'] | PredictionMarketArgs['tuple']['create_event(string,uint64)uint64']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'create_event(string,uint64)uint64' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.name, params.args.endTime],
    }
  }
  /**
   * Constructs a no op call for the place_bet(uint64,bool,pay)void ABI method
   *
  * Place a bet on an event
  This method expects to be called as part of an atomic transaction group where the first transaction is a payment to the application account.
  Args:     event_id: ID of the event to bet on     outcome: True for YES, False for NO     payment: Payment transaction with the bet amount      Raises:     Error if event doesn't exist     Error if event is already resolved     Error if betting period has ended     Error if payment amount is 0     Error if payment receiver is not the application account

   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static placeBet(params: CallParams<PredictionMarketArgs['obj']['place_bet(uint64,bool,pay)void'] | PredictionMarketArgs['tuple']['place_bet(uint64,bool,pay)void']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'place_bet(uint64,bool,pay)void' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.eventId, params.args.outcome, params.args.payment],
    }
  }
  /**
   * Constructs a no op call for the resolve_event(uint64,bool)void ABI method
   *
  * Resolve an event with the final outcome (admin only)
  Admin can resolve early if needed (no endTime check)
  Args:     event_id: ID of the event to resolve     outcome: True for YES, False for NO      Raises:     Error if caller is not admin     Error if event doesn't exist     Error if event is already resolved

   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static resolveEvent(params: CallParams<PredictionMarketArgs['obj']['resolve_event(uint64,bool)void'] | PredictionMarketArgs['tuple']['resolve_event(uint64,bool)void']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'resolve_event(uint64,bool)void' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.eventId, params.args.outcome],
    }
  }
  /**
   * Constructs a no op call for the claim_winnings(uint64)void ABI method
   *
  * Claim winnings for a winning bet
  Sends payout from app account to the bettor via inner transaction
  Args:     bet_id: ID of the bet to claim      Raises:     Error if bet doesn't exist     Error if caller is not the bettor     Error if winnings already claimed     Error if event not resolved     Error if bet did not win

   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static claimWinnings(params: CallParams<PredictionMarketArgs['obj']['claim_winnings(uint64)void'] | PredictionMarketArgs['tuple']['claim_winnings(uint64)void']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'claim_winnings(uint64)void' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.betId],
    }
  }
  /**
   * Constructs a no op call for the get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[] ABI method
   *
   * Get all bets for a user
   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getUserBets(params: CallParams<PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.user],
    }
  }
  /**
   * Constructs a no op call for the get_total_bets(uint64)(uint64,uint64) ABI method
   *
  * Get total bet counts for an event
  Args:     event_id: ID of the event      Returns:     Tuple of (yes_bets, no_bets)      Raises:     Error if event doesn't exist

   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getTotalBets(params: CallParams<PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)'] | PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_total_bets(uint64)(uint64,uint64)' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.eventId],
    }
  }
  /**
   * Constructs a no op call for the get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64) ABI method
   *
  * Get event details
  Args:     event_id: ID of the event      Returns:     Event struct with all event data      Raises:     Error if event doesn't exist

   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getEvent(params: CallParams<PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)' as const,
      args: Array.isArray(params.args) ? params.args : [params.args.eventId],
    }
  }
  /**
   * Constructs a no op call for the get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[] ABI method
   *
   * Get all events
   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getAllEvents(params: CallParams<PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]' as const,
      args: Array.isArray(params.args) ? params.args : [],
    }
  }
  /**
   * Constructs a no op call for the get_contract_balance()uint64 ABI method
   *
   * Get contract/app account balance
   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getContractBalance(params: CallParams<PredictionMarketArgs['obj']['get_contract_balance()uint64'] | PredictionMarketArgs['tuple']['get_contract_balance()uint64']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_contract_balance()uint64' as const,
      args: Array.isArray(params.args) ? params.args : [],
    }
  }
  /**
   * Constructs a no op call for the emergency_withdraw()void ABI method
   *
  * Emergency withdraw function for admin
  Transfers entire app balance to admin account
  Raises:     Error if caller is not admin

   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static emergencyWithdraw(params: CallParams<PredictionMarketArgs['obj']['emergency_withdraw()void'] | PredictionMarketArgs['tuple']['emergency_withdraw()void']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'emergency_withdraw()void' as const,
      args: Array.isArray(params.args) ? params.args : [],
    }
  }
  /**
   * Constructs a no op call for the get_admin()address ABI method
   *
   * Get the admin address
   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getAdmin(params: CallParams<PredictionMarketArgs['obj']['get_admin()address'] | PredictionMarketArgs['tuple']['get_admin()address']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_admin()address' as const,
      args: Array.isArray(params.args) ? params.args : [],
    }
  }
  /**
   * Constructs a no op call for the get_event_counter()uint64 ABI method
   *
   * Get the current event counter
   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getEventCounter(params: CallParams<PredictionMarketArgs['obj']['get_event_counter()uint64'] | PredictionMarketArgs['tuple']['get_event_counter()uint64']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_event_counter()uint64' as const,
      args: Array.isArray(params.args) ? params.args : [],
    }
  }
  /**
   * Constructs a no op call for the get_bet_counter()uint64 ABI method
   *
   * Get the current bet counter
   *
   * @param params Parameters for the call
   * @returns An `AppClientMethodCallParams` object for the call
   */
  static getBetCounter(params: CallParams<PredictionMarketArgs['obj']['get_bet_counter()uint64'] | PredictionMarketArgs['tuple']['get_bet_counter()uint64']> & CallOnComplete): AppClientMethodCallParams & CallOnComplete {
    return {
      ...params,
      method: 'get_bet_counter()uint64' as const,
      args: Array.isArray(params.args) ? params.args : [],
    }
  }
}

/**
 * A factory to create and deploy one or more instance of the PredictionMarket smart contract and to create one or more app clients to interact with those (or other) app instances
 */
export class PredictionMarketFactory {
  /**
   * The underlying `AppFactory` for when you want to have more flexibility
   */
  public readonly appFactory: _AppFactory

  /**
   * Creates a new instance of `PredictionMarketFactory`
   *
   * @param params The parameters to initialise the app factory with
   */
  constructor(params: Omit<AppFactoryParams, 'appSpec'>) {
    this.appFactory = new _AppFactory({
      ...params,
      appSpec: APP_SPEC,
    })
  }
  
  /** The name of the app (from the ARC-32 / ARC-56 app spec or override). */
  public get appName() {
    return this.appFactory.appName
  }
  
  /** The ARC-56 app spec being used */
  get appSpec() {
    return APP_SPEC
  }
  
  /** A reference to the underlying `AlgorandClient` this app factory is using. */
  public get algorand(): AlgorandClient {
    return this.appFactory.algorand
  }
  
  /**
   * Returns a new `AppClient` client for an app instance of the given ID.
   *
   * Automatically populates appName, defaultSender and source maps from the factory
   * if not specified in the params.
   * @param params The parameters to create the app client
   * @returns The `AppClient`
   */
  public getAppClientById(params: AppFactoryAppClientParams) {
    return new PredictionMarketClient(this.appFactory.getAppClientById(params))
  }
  
  /**
   * Returns a new `AppClient` client, resolving the app by creator address and name
   * using AlgoKit app deployment semantics (i.e. looking for the app creation transaction note).
   *
   * Automatically populates appName, defaultSender and source maps from the factory
   * if not specified in the params.
   * @param params The parameters to create the app client
   * @returns The `AppClient`
   */
  public async getAppClientByCreatorAndName(
    params: AppFactoryResolveAppClientByCreatorAndNameParams,
  ) {
    return new PredictionMarketClient(await this.appFactory.getAppClientByCreatorAndName(params))
  }

  /**
   * Idempotently deploys the PredictionMarket smart contract.
   *
   * @param params The arguments for the contract calls and any additional parameters for the call
   * @returns The deployment result
   */
  public async deploy(params: PredictionMarketDeployParams = {}) {
    const result = await this.appFactory.deploy({
      ...params,
      createParams: params.createParams?.method ? PredictionMarketParamsFactory.create._resolveByMethod(params.createParams) : params.createParams ? params.createParams as (PredictionMarketCreateCallParams & { args: Uint8Array[] }) : undefined,
    })
    return { result: result.result, appClient: new PredictionMarketClient(result.appClient) }
  }

  /**
   * Get parameters to create transactions (create and deploy related calls) for the current app. A good mental model for this is that these parameters represent a deferred transaction creation.
   */
  readonly params = {
    /**
     * Gets available create methods
     */
    create: {
      /**
       * Creates a new instance of the PredictionMarket smart contract using the create_application(address)void ABI method.
       *
      * Initialize the application
      Sets the admin address and initializes counters

       *
       * @param params The params for the smart contract call
       * @returns The create params
       */
      createApplication: (params: CallParams<PredictionMarketArgs['obj']['create_application(address)void'] | PredictionMarketArgs['tuple']['create_application(address)void']> & AppClientCompilationParams & CreateSchema & {onComplete?: OnApplicationComplete.NoOpOC}) => {
        return this.appFactory.params.create(PredictionMarketParamsFactory.create.createApplication(params))
      },
    },

  }

  /**
   * Create transactions for the current app
   */
  readonly createTransaction = {
    /**
     * Gets available create methods
     */
    create: {
      /**
       * Creates a new instance of the PredictionMarket smart contract using the create_application(address)void ABI method.
       *
      * Initialize the application
      Sets the admin address and initializes counters

       *
       * @param params The params for the smart contract call
       * @returns The create transaction
       */
      createApplication: (params: CallParams<PredictionMarketArgs['obj']['create_application(address)void'] | PredictionMarketArgs['tuple']['create_application(address)void']> & AppClientCompilationParams & CreateSchema & {onComplete?: OnApplicationComplete.NoOpOC}) => {
        return this.appFactory.createTransaction.create(PredictionMarketParamsFactory.create.createApplication(params))
      },
    },

  }

  /**
   * Send calls to the current app
   */
  readonly send = {
    /**
     * Gets available create methods
     */
    create: {
      /**
       * Creates a new instance of the PredictionMarket smart contract using an ABI method call using the create_application(address)void ABI method.
       *
      * Initialize the application
      Sets the admin address and initializes counters

       *
       * @param params The params for the smart contract call
       * @returns The create result
       */
      createApplication: async (params: CallParams<PredictionMarketArgs['obj']['create_application(address)void'] | PredictionMarketArgs['tuple']['create_application(address)void']> & AppClientCompilationParams & CreateSchema & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
        const result = await this.appFactory.send.create(PredictionMarketParamsFactory.create.createApplication(params))
        return { result: { ...result.result, return: result.result.return as unknown as (undefined | PredictionMarketReturns['create_application(address)void']) }, appClient: new PredictionMarketClient(result.appClient) }
      },
    },

  }

}
/**
 * A client to make calls to the PredictionMarket smart contract
 */
export class PredictionMarketClient {
  /**
   * The underlying `AppClient` for when you want to have more flexibility
   */
  public readonly appClient: _AppClient

  /**
   * Creates a new instance of `PredictionMarketClient`
   *
   * @param appClient An `AppClient` instance which has been created with the PredictionMarket app spec
   */
  constructor(appClient: _AppClient)
  /**
   * Creates a new instance of `PredictionMarketClient`
   *
   * @param params The parameters to initialise the app client with
   */
  constructor(params: Omit<AppClientParams, 'appSpec'>)
  constructor(appClientOrParams: _AppClient | Omit<AppClientParams, 'appSpec'>) {
    this.appClient = appClientOrParams instanceof _AppClient ? appClientOrParams : new _AppClient({
      ...appClientOrParams,
      appSpec: APP_SPEC,
    })
  }
  
  /**
   * Checks for decode errors on the given return value and maps the return value to the return type for the given method
   * @returns The typed return value or undefined if there was no value
   */
  decodeReturnValue<TSignature extends PredictionMarketNonVoidMethodSignatures>(method: TSignature, returnValue: ABIReturn | undefined) {
    return returnValue !== undefined ? getArc56ReturnValue<MethodReturn<TSignature>>(returnValue, this.appClient.getABIMethod(method), APP_SPEC.structs) : undefined
  }
  
  /**
   * Returns a new `PredictionMarketClient` client, resolving the app by creator address and name
   * using AlgoKit app deployment semantics (i.e. looking for the app creation transaction note).
   * @param params The parameters to create the app client
   */
  public static async fromCreatorAndName(params: Omit<ResolveAppClientByCreatorAndName, 'appSpec'>): Promise<PredictionMarketClient> {
    return new PredictionMarketClient(await _AppClient.fromCreatorAndName({...params, appSpec: APP_SPEC}))
  }
  
  /**
   * Returns an `PredictionMarketClient` instance for the current network based on
   * pre-determined network-specific app IDs specified in the ARC-56 app spec.
   *
   * If no IDs are in the app spec or the network isn't recognised, an error is thrown.
   * @param params The parameters to create the app client
   */
  static async fromNetwork(
    params: Omit<ResolveAppClientByNetwork, 'appSpec'>
  ): Promise<PredictionMarketClient> {
    return new PredictionMarketClient(await _AppClient.fromNetwork({...params, appSpec: APP_SPEC}))
  }
  
  /** The ID of the app instance this client is linked to. */
  public get appId() {
    return this.appClient.appId
  }
  
  /** The app address of the app instance this client is linked to. */
  public get appAddress() {
    return this.appClient.appAddress
  }
  
  /** The name of the app. */
  public get appName() {
    return this.appClient.appName
  }
  
  /** The ARC-56 app spec being used */
  public get appSpec() {
    return this.appClient.appSpec
  }
  
  /** A reference to the underlying `AlgorandClient` this app client is using. */
  public get algorand(): AlgorandClient {
    return this.appClient.algorand
  }

  /**
   * Get parameters to create transactions for the current app. A good mental model for this is that these parameters represent a deferred transaction creation.
   */
  readonly params = {
    /**
     * Makes a clear_state call to an existing instance of the PredictionMarket smart contract.
     *
     * @param params The params for the bare (raw) call
     * @returns The clearState result
     */
    clearState: (params?: Expand<AppClientBareCallParams>) => {
      return this.appClient.params.bare.clearState(params)
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `create_event(string,uint64)uint64` ABI method.
     *
    * Create a new prediction event (admin only)
    Args:     name: Name/description of the event     end_time: Unix timestamp when betting ends      Returns:     The event ID of the newly created event      Raises:     Error if caller is not admin     Error if end_time is not in the future

     *
     * @param params The params for the smart contract call
     * @returns The call params
     */
    createEvent: (params: CallParams<PredictionMarketArgs['obj']['create_event(string,uint64)uint64'] | PredictionMarketArgs['tuple']['create_event(string,uint64)uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.createEvent(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `place_bet(uint64,bool,pay)void` ABI method.
     *
    * Place a bet on an event
    This method expects to be called as part of an atomic transaction group where the first transaction is a payment to the application account.
    Args:     event_id: ID of the event to bet on     outcome: True for YES, False for NO     payment: Payment transaction with the bet amount      Raises:     Error if event doesn't exist     Error if event is already resolved     Error if betting period has ended     Error if payment amount is 0     Error if payment receiver is not the application account

     *
     * @param params The params for the smart contract call
     * @returns The call params
     */
    placeBet: (params: CallParams<PredictionMarketArgs['obj']['place_bet(uint64,bool,pay)void'] | PredictionMarketArgs['tuple']['place_bet(uint64,bool,pay)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.placeBet(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `resolve_event(uint64,bool)void` ABI method.
     *
    * Resolve an event with the final outcome (admin only)
    Admin can resolve early if needed (no endTime check)
    Args:     event_id: ID of the event to resolve     outcome: True for YES, False for NO      Raises:     Error if caller is not admin     Error if event doesn't exist     Error if event is already resolved

     *
     * @param params The params for the smart contract call
     * @returns The call params
     */
    resolveEvent: (params: CallParams<PredictionMarketArgs['obj']['resolve_event(uint64,bool)void'] | PredictionMarketArgs['tuple']['resolve_event(uint64,bool)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.resolveEvent(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `claim_winnings(uint64)void` ABI method.
     *
    * Claim winnings for a winning bet
    Sends payout from app account to the bettor via inner transaction
    Args:     bet_id: ID of the bet to claim      Raises:     Error if bet doesn't exist     Error if caller is not the bettor     Error if winnings already claimed     Error if event not resolved     Error if bet did not win

     *
     * @param params The params for the smart contract call
     * @returns The call params
     */
    claimWinnings: (params: CallParams<PredictionMarketArgs['obj']['claim_winnings(uint64)void'] | PredictionMarketArgs['tuple']['claim_winnings(uint64)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.claimWinnings(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get all bets for a user
     *
     * @param params The params for the smart contract call
     * @returns The call params: Array of Bet structs for the user
     */
    getUserBets: (params: CallParams<PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getUserBets(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_total_bets(uint64)(uint64,uint64)` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
    * Get total bet counts for an event
    Args:     event_id: ID of the event      Returns:     Tuple of (yes_bets, no_bets)      Raises:     Error if event doesn't exist

     *
     * @param params The params for the smart contract call
     * @returns The call params
     */
    getTotalBets: (params: CallParams<PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)'] | PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getTotalBets(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
    * Get event details
    Args:     event_id: ID of the event      Returns:     Event struct with all event data      Raises:     Error if event doesn't exist

     *
     * @param params The params for the smart contract call
     * @returns The call params
     */
    getEvent: (params: CallParams<PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getEvent(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get all events
     *
     * @param params The params for the smart contract call
     * @returns The call params: Array of all Event structs
     */
    getAllEvents: (params: CallParams<PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getAllEvents(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_contract_balance()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get contract/app account balance
     *
     * @param params The params for the smart contract call
     * @returns The call params: Current balance of the application account in microAlgos
     */
    getContractBalance: (params: CallParams<PredictionMarketArgs['obj']['get_contract_balance()uint64'] | PredictionMarketArgs['tuple']['get_contract_balance()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getContractBalance(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `emergency_withdraw()void` ABI method.
     *
    * Emergency withdraw function for admin
    Transfers entire app balance to admin account
    Raises:     Error if caller is not admin

     *
     * @param params The params for the smart contract call
     * @returns The call params
     */
    emergencyWithdraw: (params: CallParams<PredictionMarketArgs['obj']['emergency_withdraw()void'] | PredictionMarketArgs['tuple']['emergency_withdraw()void']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.emergencyWithdraw(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_admin()address` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the admin address
     *
     * @param params The params for the smart contract call
     * @returns The call params: Address of the admin account
     */
    getAdmin: (params: CallParams<PredictionMarketArgs['obj']['get_admin()address'] | PredictionMarketArgs['tuple']['get_admin()address']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getAdmin(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_event_counter()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the current event counter
     *
     * @param params The params for the smart contract call
     * @returns The call params: Number of events created
     */
    getEventCounter: (params: CallParams<PredictionMarketArgs['obj']['get_event_counter()uint64'] | PredictionMarketArgs['tuple']['get_event_counter()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getEventCounter(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_bet_counter()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the current bet counter
     *
     * @param params The params for the smart contract call
     * @returns The call params: Number of bets placed
     */
    getBetCounter: (params: CallParams<PredictionMarketArgs['obj']['get_bet_counter()uint64'] | PredictionMarketArgs['tuple']['get_bet_counter()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.params.call(PredictionMarketParamsFactory.getBetCounter(params))
    },

  }

  /**
   * Create transactions for the current app
   */
  readonly createTransaction = {
    /**
     * Makes a clear_state call to an existing instance of the PredictionMarket smart contract.
     *
     * @param params The params for the bare (raw) call
     * @returns The clearState result
     */
    clearState: (params?: Expand<AppClientBareCallParams>) => {
      return this.appClient.createTransaction.bare.clearState(params)
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `create_event(string,uint64)uint64` ABI method.
     *
    * Create a new prediction event (admin only)
    Args:     name: Name/description of the event     end_time: Unix timestamp when betting ends      Returns:     The event ID of the newly created event      Raises:     Error if caller is not admin     Error if end_time is not in the future

     *
     * @param params The params for the smart contract call
     * @returns The call transaction
     */
    createEvent: (params: CallParams<PredictionMarketArgs['obj']['create_event(string,uint64)uint64'] | PredictionMarketArgs['tuple']['create_event(string,uint64)uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.createEvent(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `place_bet(uint64,bool,pay)void` ABI method.
     *
    * Place a bet on an event
    This method expects to be called as part of an atomic transaction group where the first transaction is a payment to the application account.
    Args:     event_id: ID of the event to bet on     outcome: True for YES, False for NO     payment: Payment transaction with the bet amount      Raises:     Error if event doesn't exist     Error if event is already resolved     Error if betting period has ended     Error if payment amount is 0     Error if payment receiver is not the application account

     *
     * @param params The params for the smart contract call
     * @returns The call transaction
     */
    placeBet: (params: CallParams<PredictionMarketArgs['obj']['place_bet(uint64,bool,pay)void'] | PredictionMarketArgs['tuple']['place_bet(uint64,bool,pay)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.placeBet(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `resolve_event(uint64,bool)void` ABI method.
     *
    * Resolve an event with the final outcome (admin only)
    Admin can resolve early if needed (no endTime check)
    Args:     event_id: ID of the event to resolve     outcome: True for YES, False for NO      Raises:     Error if caller is not admin     Error if event doesn't exist     Error if event is already resolved

     *
     * @param params The params for the smart contract call
     * @returns The call transaction
     */
    resolveEvent: (params: CallParams<PredictionMarketArgs['obj']['resolve_event(uint64,bool)void'] | PredictionMarketArgs['tuple']['resolve_event(uint64,bool)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.resolveEvent(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `claim_winnings(uint64)void` ABI method.
     *
    * Claim winnings for a winning bet
    Sends payout from app account to the bettor via inner transaction
    Args:     bet_id: ID of the bet to claim      Raises:     Error if bet doesn't exist     Error if caller is not the bettor     Error if winnings already claimed     Error if event not resolved     Error if bet did not win

     *
     * @param params The params for the smart contract call
     * @returns The call transaction
     */
    claimWinnings: (params: CallParams<PredictionMarketArgs['obj']['claim_winnings(uint64)void'] | PredictionMarketArgs['tuple']['claim_winnings(uint64)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.claimWinnings(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get all bets for a user
     *
     * @param params The params for the smart contract call
     * @returns The call transaction: Array of Bet structs for the user
     */
    getUserBets: (params: CallParams<PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getUserBets(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_total_bets(uint64)(uint64,uint64)` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
    * Get total bet counts for an event
    Args:     event_id: ID of the event      Returns:     Tuple of (yes_bets, no_bets)      Raises:     Error if event doesn't exist

     *
     * @param params The params for the smart contract call
     * @returns The call transaction
     */
    getTotalBets: (params: CallParams<PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)'] | PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getTotalBets(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
    * Get event details
    Args:     event_id: ID of the event      Returns:     Event struct with all event data      Raises:     Error if event doesn't exist

     *
     * @param params The params for the smart contract call
     * @returns The call transaction
     */
    getEvent: (params: CallParams<PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']> & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getEvent(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get all events
     *
     * @param params The params for the smart contract call
     * @returns The call transaction: Array of all Event structs
     */
    getAllEvents: (params: CallParams<PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getAllEvents(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_contract_balance()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get contract/app account balance
     *
     * @param params The params for the smart contract call
     * @returns The call transaction: Current balance of the application account in microAlgos
     */
    getContractBalance: (params: CallParams<PredictionMarketArgs['obj']['get_contract_balance()uint64'] | PredictionMarketArgs['tuple']['get_contract_balance()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getContractBalance(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `emergency_withdraw()void` ABI method.
     *
    * Emergency withdraw function for admin
    Transfers entire app balance to admin account
    Raises:     Error if caller is not admin

     *
     * @param params The params for the smart contract call
     * @returns The call transaction
     */
    emergencyWithdraw: (params: CallParams<PredictionMarketArgs['obj']['emergency_withdraw()void'] | PredictionMarketArgs['tuple']['emergency_withdraw()void']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.emergencyWithdraw(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_admin()address` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the admin address
     *
     * @param params The params for the smart contract call
     * @returns The call transaction: Address of the admin account
     */
    getAdmin: (params: CallParams<PredictionMarketArgs['obj']['get_admin()address'] | PredictionMarketArgs['tuple']['get_admin()address']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getAdmin(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_event_counter()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the current event counter
     *
     * @param params The params for the smart contract call
     * @returns The call transaction: Number of events created
     */
    getEventCounter: (params: CallParams<PredictionMarketArgs['obj']['get_event_counter()uint64'] | PredictionMarketArgs['tuple']['get_event_counter()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getEventCounter(params))
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_bet_counter()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the current bet counter
     *
     * @param params The params for the smart contract call
     * @returns The call transaction: Number of bets placed
     */
    getBetCounter: (params: CallParams<PredictionMarketArgs['obj']['get_bet_counter()uint64'] | PredictionMarketArgs['tuple']['get_bet_counter()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      return this.appClient.createTransaction.call(PredictionMarketParamsFactory.getBetCounter(params))
    },

  }

  /**
   * Send calls to the current app
   */
  readonly send = {
    /**
     * Makes a clear_state call to an existing instance of the PredictionMarket smart contract.
     *
     * @param params The params for the bare (raw) call
     * @returns The clearState result
     */
    clearState: (params?: Expand<AppClientBareCallParams & SendParams>) => {
      return this.appClient.send.bare.clearState(params)
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `create_event(string,uint64)uint64` ABI method.
     *
    * Create a new prediction event (admin only)
    Args:     name: Name/description of the event     end_time: Unix timestamp when betting ends      Returns:     The event ID of the newly created event      Raises:     Error if caller is not admin     Error if end_time is not in the future

     *
     * @param params The params for the smart contract call
     * @returns The call result
     */
    createEvent: async (params: CallParams<PredictionMarketArgs['obj']['create_event(string,uint64)uint64'] | PredictionMarketArgs['tuple']['create_event(string,uint64)uint64']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.createEvent(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['create_event(string,uint64)uint64'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `place_bet(uint64,bool,pay)void` ABI method.
     *
    * Place a bet on an event
    This method expects to be called as part of an atomic transaction group where the first transaction is a payment to the application account.
    Args:     event_id: ID of the event to bet on     outcome: True for YES, False for NO     payment: Payment transaction with the bet amount      Raises:     Error if event doesn't exist     Error if event is already resolved     Error if betting period has ended     Error if payment amount is 0     Error if payment receiver is not the application account

     *
     * @param params The params for the smart contract call
     * @returns The call result
     */
    placeBet: async (params: CallParams<PredictionMarketArgs['obj']['place_bet(uint64,bool,pay)void'] | PredictionMarketArgs['tuple']['place_bet(uint64,bool,pay)void']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.placeBet(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['place_bet(uint64,bool,pay)void'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `resolve_event(uint64,bool)void` ABI method.
     *
    * Resolve an event with the final outcome (admin only)
    Admin can resolve early if needed (no endTime check)
    Args:     event_id: ID of the event to resolve     outcome: True for YES, False for NO      Raises:     Error if caller is not admin     Error if event doesn't exist     Error if event is already resolved

     *
     * @param params The params for the smart contract call
     * @returns The call result
     */
    resolveEvent: async (params: CallParams<PredictionMarketArgs['obj']['resolve_event(uint64,bool)void'] | PredictionMarketArgs['tuple']['resolve_event(uint64,bool)void']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.resolveEvent(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['resolve_event(uint64,bool)void'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `claim_winnings(uint64)void` ABI method.
     *
    * Claim winnings for a winning bet
    Sends payout from app account to the bettor via inner transaction
    Args:     bet_id: ID of the bet to claim      Raises:     Error if bet doesn't exist     Error if caller is not the bettor     Error if winnings already claimed     Error if event not resolved     Error if bet did not win

     *
     * @param params The params for the smart contract call
     * @returns The call result
     */
    claimWinnings: async (params: CallParams<PredictionMarketArgs['obj']['claim_winnings(uint64)void'] | PredictionMarketArgs['tuple']['claim_winnings(uint64)void']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.claimWinnings(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['claim_winnings(uint64)void'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get all bets for a user
     *
     * @param params The params for the smart contract call
     * @returns The call result: Array of Bet structs for the user
     */
    getUserBets: async (params: CallParams<PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getUserBets(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_total_bets(uint64)(uint64,uint64)` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
    * Get total bet counts for an event
    Args:     event_id: ID of the event      Returns:     Tuple of (yes_bets, no_bets)      Raises:     Error if event doesn't exist

     *
     * @param params The params for the smart contract call
     * @returns The call result
     */
    getTotalBets: async (params: CallParams<PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)'] | PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getTotalBets(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_total_bets(uint64)(uint64,uint64)'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
    * Get event details
    Args:     event_id: ID of the event      Returns:     Event struct with all event data      Raises:     Error if event doesn't exist

     *
     * @param params The params for the smart contract call
     * @returns The call result
     */
    getEvent: async (params: CallParams<PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getEvent(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get all events
     *
     * @param params The params for the smart contract call
     * @returns The call result: Array of all Event structs
     */
    getAllEvents: async (params: CallParams<PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getAllEvents(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_contract_balance()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get contract/app account balance
     *
     * @param params The params for the smart contract call
     * @returns The call result: Current balance of the application account in microAlgos
     */
    getContractBalance: async (params: CallParams<PredictionMarketArgs['obj']['get_contract_balance()uint64'] | PredictionMarketArgs['tuple']['get_contract_balance()uint64']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getContractBalance(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_contract_balance()uint64'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `emergency_withdraw()void` ABI method.
     *
    * Emergency withdraw function for admin
    Transfers entire app balance to admin account
    Raises:     Error if caller is not admin

     *
     * @param params The params for the smart contract call
     * @returns The call result
     */
    emergencyWithdraw: async (params: CallParams<PredictionMarketArgs['obj']['emergency_withdraw()void'] | PredictionMarketArgs['tuple']['emergency_withdraw()void']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.emergencyWithdraw(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['emergency_withdraw()void'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_admin()address` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the admin address
     *
     * @param params The params for the smart contract call
     * @returns The call result: Address of the admin account
     */
    getAdmin: async (params: CallParams<PredictionMarketArgs['obj']['get_admin()address'] | PredictionMarketArgs['tuple']['get_admin()address']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getAdmin(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_admin()address'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_event_counter()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the current event counter
     *
     * @param params The params for the smart contract call
     * @returns The call result: Number of events created
     */
    getEventCounter: async (params: CallParams<PredictionMarketArgs['obj']['get_event_counter()uint64'] | PredictionMarketArgs['tuple']['get_event_counter()uint64']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getEventCounter(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_event_counter()uint64'])}
    },

    /**
     * Makes a call to the PredictionMarket smart contract using the `get_bet_counter()uint64` ABI method.
     * 
     * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
     *
     * Get the current bet counter
     *
     * @param params The params for the smart contract call
     * @returns The call result: Number of bets placed
     */
    getBetCounter: async (params: CallParams<PredictionMarketArgs['obj']['get_bet_counter()uint64'] | PredictionMarketArgs['tuple']['get_bet_counter()uint64']> & SendParams & {onComplete?: OnApplicationComplete.NoOpOC} = {args: []}) => {
      const result = await this.appClient.send.call(PredictionMarketParamsFactory.getBetCounter(params))
      return {...result, return: result.return as unknown as (undefined | PredictionMarketReturns['get_bet_counter()uint64'])}
    },

  }

  /**
   * Clone this app client with different params
   *
   * @param params The params to use for the the cloned app client. Omit a param to keep the original value. Set a param to override the original value. Setting to undefined will clear the original value.
   * @returns A new app client with the altered params
   */
  public clone(params: CloneAppClientParams) {
    return new PredictionMarketClient(this.appClient.clone(params))
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
   * Get all bets for a user
   *
   * @param params The params for the smart contract call
   * @returns The call result: Array of Bet structs for the user
   */
  async getUserBets(params: CallParams<PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']>) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getUserBets(params))
    return result.return as unknown as PredictionMarketReturns['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_total_bets(uint64)(uint64,uint64)` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
  * Get total bet counts for an event
  Args:     event_id: ID of the event      Returns:     Tuple of (yes_bets, no_bets)      Raises:     Error if event doesn't exist

   *
   * @param params The params for the smart contract call
   * @returns The call result
   */
  async getTotalBets(params: CallParams<PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)'] | PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']>) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getTotalBets(params))
    return result.return as unknown as PredictionMarketReturns['get_total_bets(uint64)(uint64,uint64)']
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
  * Get event details
  Args:     event_id: ID of the event      Returns:     Event struct with all event data      Raises:     Error if event doesn't exist

   *
   * @param params The params for the smart contract call
   * @returns The call result
   */
  async getEvent(params: CallParams<PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']>) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getEvent(params))
    return result.return as unknown as PredictionMarketReturns['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
   * Get all events
   *
   * @param params The params for the smart contract call
   * @returns The call result: Array of all Event structs
   */
  async getAllEvents(params: CallParams<PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']> = {args: []}) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getAllEvents(params))
    return result.return as unknown as PredictionMarketReturns['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_contract_balance()uint64` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
   * Get contract/app account balance
   *
   * @param params The params for the smart contract call
   * @returns The call result: Current balance of the application account in microAlgos
   */
  async getContractBalance(params: CallParams<PredictionMarketArgs['obj']['get_contract_balance()uint64'] | PredictionMarketArgs['tuple']['get_contract_balance()uint64']> = {args: []}) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getContractBalance(params))
    return result.return as unknown as PredictionMarketReturns['get_contract_balance()uint64']
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_admin()address` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
   * Get the admin address
   *
   * @param params The params for the smart contract call
   * @returns The call result: Address of the admin account
   */
  async getAdmin(params: CallParams<PredictionMarketArgs['obj']['get_admin()address'] | PredictionMarketArgs['tuple']['get_admin()address']> = {args: []}) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getAdmin(params))
    return result.return as unknown as PredictionMarketReturns['get_admin()address']
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_event_counter()uint64` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
   * Get the current event counter
   *
   * @param params The params for the smart contract call
   * @returns The call result: Number of events created
   */
  async getEventCounter(params: CallParams<PredictionMarketArgs['obj']['get_event_counter()uint64'] | PredictionMarketArgs['tuple']['get_event_counter()uint64']> = {args: []}) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getEventCounter(params))
    return result.return as unknown as PredictionMarketReturns['get_event_counter()uint64']
  }

  /**
   * Makes a readonly (simulated) call to the PredictionMarket smart contract using the `get_bet_counter()uint64` ABI method.
   * 
   * This method is a readonly method; calling it with onComplete of NoOp will result in a simulated transaction rather than a real transaction.
   *
   * Get the current bet counter
   *
   * @param params The params for the smart contract call
   * @returns The call result: Number of bets placed
   */
  async getBetCounter(params: CallParams<PredictionMarketArgs['obj']['get_bet_counter()uint64'] | PredictionMarketArgs['tuple']['get_bet_counter()uint64']> = {args: []}) {
    const result = await this.appClient.send.call(PredictionMarketParamsFactory.getBetCounter(params))
    return result.return as unknown as PredictionMarketReturns['get_bet_counter()uint64']
  }

  /**
   * Methods to access state for the current PredictionMarket app
   */
  state = {
    /**
     * Methods to access global state for the current PredictionMarket app
     */
    global: {
      /**
       * Get all current keyed values from global state
       */
      getAll: async (): Promise<Partial<Expand<GlobalKeysState>>> => {
        const result = await this.appClient.state.global.getAll()
        return {
          admin: result.admin,
          eventCounter: result.event_counter,
          betCounter: result.bet_counter,
        }
      },
      /**
       * Get the current value of the admin key in global state
       */
      admin: async (): Promise<string | undefined> => { return (await this.appClient.state.global.getValue("admin")) as string | undefined },
      /**
       * Get the current value of the event_counter key in global state
       */
      eventCounter: async (): Promise<bigint | undefined> => { return (await this.appClient.state.global.getValue("event_counter")) as bigint | undefined },
      /**
       * Get the current value of the bet_counter key in global state
       */
      betCounter: async (): Promise<bigint | undefined> => { return (await this.appClient.state.global.getValue("bet_counter")) as bigint | undefined },
    },
    /**
     * Methods to access box state for the current PredictionMarket app
     */
    box: {
      /**
       * Get all current keyed values from box state
       */
      getAll: async (): Promise<Partial<Expand<BoxKeysState>>> => {
        const result = await this.appClient.state.box.getAll()
        return {
        }
      },
      /**
       * Get values from the events map in box state
       */
      events: {
        /**
         * Get all current values of the events map in box state
         */
        getMap: async (): Promise<Map<bigint, EventStruct>> => { return (await this.appClient.state.box.getMap("events")) as Map<bigint, EventStruct> },
        /**
         * Get a current value of the events map by key from box state
         */
        value: async (key: bigint | number): Promise<EventStruct | undefined> => { return await this.appClient.state.box.getMapValue("events", key) as EventStruct | undefined },
      },
      /**
       * Get values from the bets map in box state
       */
      bets: {
        /**
         * Get all current values of the bets map in box state
         */
        getMap: async (): Promise<Map<bigint, BetStruct>> => { return (await this.appClient.state.box.getMap("bets")) as Map<bigint, BetStruct> },
        /**
         * Get a current value of the bets map by key from box state
         */
        value: async (key: bigint | number): Promise<BetStruct | undefined> => { return await this.appClient.state.box.getMapValue("bets", key) as BetStruct | undefined },
      },
      /**
       * Get values from the user_bets map in box state
       */
      userBets: {
        /**
         * Get all current values of the user_bets map in box state
         */
        getMap: async (): Promise<Map<string, bigint[]>> => { return (await this.appClient.state.box.getMap("user_bets")) as Map<string, bigint[]> },
        /**
         * Get a current value of the user_bets map by key from box state
         */
        value: async (key: string): Promise<bigint[] | undefined> => { return await this.appClient.state.box.getMapValue("user_bets", key) as bigint[] | undefined },
      },
      /**
       * Get values from the event_bets map in box state
       */
      eventBets: {
        /**
         * Get all current values of the event_bets map in box state
         */
        getMap: async (): Promise<Map<bigint, bigint[]>> => { return (await this.appClient.state.box.getMap("event_bets")) as Map<bigint, bigint[]> },
        /**
         * Get a current value of the event_bets map by key from box state
         */
        value: async (key: bigint | number): Promise<bigint[] | undefined> => { return await this.appClient.state.box.getMapValue("event_bets", key) as bigint[] | undefined },
      },
    },
  }

  public newGroup(): PredictionMarketComposer {
    const client = this
    const composer = this.algorand.newGroup()
    let promiseChain:Promise<unknown> = Promise.resolve()
    const resultMappers: Array<undefined | ((x: ABIReturn | undefined) => any)> = []
    return {
      /**
       * Add a create_event(string,uint64)uint64 method call against the PredictionMarket contract
       */
      createEvent(params: CallParams<PredictionMarketArgs['obj']['create_event(string,uint64)uint64'] | PredictionMarketArgs['tuple']['create_event(string,uint64)uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.createEvent(params)))
        resultMappers.push((v) => client.decodeReturnValue('create_event(string,uint64)uint64', v))
        return this
      },
      /**
       * Add a place_bet(uint64,bool,pay)void method call against the PredictionMarket contract
       */
      placeBet(params: CallParams<PredictionMarketArgs['obj']['place_bet(uint64,bool,pay)void'] | PredictionMarketArgs['tuple']['place_bet(uint64,bool,pay)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.placeBet(params)))
        resultMappers.push(undefined)
        return this
      },
      /**
       * Add a resolve_event(uint64,bool)void method call against the PredictionMarket contract
       */
      resolveEvent(params: CallParams<PredictionMarketArgs['obj']['resolve_event(uint64,bool)void'] | PredictionMarketArgs['tuple']['resolve_event(uint64,bool)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.resolveEvent(params)))
        resultMappers.push(undefined)
        return this
      },
      /**
       * Add a claim_winnings(uint64)void method call against the PredictionMarket contract
       */
      claimWinnings(params: CallParams<PredictionMarketArgs['obj']['claim_winnings(uint64)void'] | PredictionMarketArgs['tuple']['claim_winnings(uint64)void']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.claimWinnings(params)))
        resultMappers.push(undefined)
        return this
      },
      /**
       * Add a get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[] method call against the PredictionMarket contract
       */
      getUserBets(params: CallParams<PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getUserBets(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]', v))
        return this
      },
      /**
       * Add a get_total_bets(uint64)(uint64,uint64) method call against the PredictionMarket contract
       */
      getTotalBets(params: CallParams<PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)'] | PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getTotalBets(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_total_bets(uint64)(uint64,uint64)', v))
        return this
      },
      /**
       * Add a get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64) method call against the PredictionMarket contract
       */
      getEvent(params: CallParams<PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getEvent(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)', v))
        return this
      },
      /**
       * Add a get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[] method call against the PredictionMarket contract
       */
      getAllEvents(params: CallParams<PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getAllEvents(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]', v))
        return this
      },
      /**
       * Add a get_contract_balance()uint64 method call against the PredictionMarket contract
       */
      getContractBalance(params: CallParams<PredictionMarketArgs['obj']['get_contract_balance()uint64'] | PredictionMarketArgs['tuple']['get_contract_balance()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getContractBalance(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_contract_balance()uint64', v))
        return this
      },
      /**
       * Add a emergency_withdraw()void method call against the PredictionMarket contract
       */
      emergencyWithdraw(params: CallParams<PredictionMarketArgs['obj']['emergency_withdraw()void'] | PredictionMarketArgs['tuple']['emergency_withdraw()void']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.emergencyWithdraw(params)))
        resultMappers.push(undefined)
        return this
      },
      /**
       * Add a get_admin()address method call against the PredictionMarket contract
       */
      getAdmin(params: CallParams<PredictionMarketArgs['obj']['get_admin()address'] | PredictionMarketArgs['tuple']['get_admin()address']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getAdmin(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_admin()address', v))
        return this
      },
      /**
       * Add a get_event_counter()uint64 method call against the PredictionMarket contract
       */
      getEventCounter(params: CallParams<PredictionMarketArgs['obj']['get_event_counter()uint64'] | PredictionMarketArgs['tuple']['get_event_counter()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getEventCounter(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_event_counter()uint64', v))
        return this
      },
      /**
       * Add a get_bet_counter()uint64 method call against the PredictionMarket contract
       */
      getBetCounter(params: CallParams<PredictionMarketArgs['obj']['get_bet_counter()uint64'] | PredictionMarketArgs['tuple']['get_bet_counter()uint64']> & {onComplete?: OnApplicationComplete.NoOpOC}) {
        promiseChain = promiseChain.then(async () => composer.addAppCallMethodCall(await client.params.getBetCounter(params)))
        resultMappers.push((v) => client.decodeReturnValue('get_bet_counter()uint64', v))
        return this
      },
      /**
       * Add a clear state call to the PredictionMarket contract
       */
      clearState(params: AppClientBareCallParams) {
        promiseChain = promiseChain.then(() => composer.addAppCall(client.params.clearState(params)))
        return this
      },
      addTransaction(txn: Transaction, signer?: TransactionSigner) {
        promiseChain = promiseChain.then(() => composer.addTransaction(txn, signer))
        return this
      },
      async composer() {
        await promiseChain
        return composer
      },
      async simulate(options?: SimulateOptions) {
        await promiseChain
        const result = await (!options ? composer.simulate() : composer.simulate(options))
        return {
          ...result,
          returns: result.returns?.map((val, i) => resultMappers[i] !== undefined ? resultMappers[i]!(val) : val.returnValue)
        }
      },
      async send(params?: SendParams) {
        await promiseChain
        const result = await composer.send(params)
        return {
          ...result,
          returns: result.returns?.map((val, i) => resultMappers[i] !== undefined ? resultMappers[i]!(val) : val.returnValue)
        }
      }
    } as unknown as PredictionMarketComposer
  }
}
export type PredictionMarketComposer<TReturns extends [...any[]] = []> = {
  /**
   * Calls the create_event(string,uint64)uint64 ABI method.
   *
  * Create a new prediction event (admin only)
  Args:     name: Name/description of the event     end_time: Unix timestamp when betting ends      Returns:     The event ID of the newly created event      Raises:     Error if caller is not admin     Error if end_time is not in the future

   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  createEvent(params?: CallParams<PredictionMarketArgs['obj']['create_event(string,uint64)uint64'] | PredictionMarketArgs['tuple']['create_event(string,uint64)uint64']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['create_event(string,uint64)uint64'] | undefined]>

  /**
   * Calls the place_bet(uint64,bool,pay)void ABI method.
   *
  * Place a bet on an event
  This method expects to be called as part of an atomic transaction group where the first transaction is a payment to the application account.
  Args:     event_id: ID of the event to bet on     outcome: True for YES, False for NO     payment: Payment transaction with the bet amount      Raises:     Error if event doesn't exist     Error if event is already resolved     Error if betting period has ended     Error if payment amount is 0     Error if payment receiver is not the application account

   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  placeBet(params?: CallParams<PredictionMarketArgs['obj']['place_bet(uint64,bool,pay)void'] | PredictionMarketArgs['tuple']['place_bet(uint64,bool,pay)void']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['place_bet(uint64,bool,pay)void'] | undefined]>

  /**
   * Calls the resolve_event(uint64,bool)void ABI method.
   *
  * Resolve an event with the final outcome (admin only)
  Admin can resolve early if needed (no endTime check)
  Args:     event_id: ID of the event to resolve     outcome: True for YES, False for NO      Raises:     Error if caller is not admin     Error if event doesn't exist     Error if event is already resolved

   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  resolveEvent(params?: CallParams<PredictionMarketArgs['obj']['resolve_event(uint64,bool)void'] | PredictionMarketArgs['tuple']['resolve_event(uint64,bool)void']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['resolve_event(uint64,bool)void'] | undefined]>

  /**
   * Calls the claim_winnings(uint64)void ABI method.
   *
  * Claim winnings for a winning bet
  Sends payout from app account to the bettor via inner transaction
  Args:     bet_id: ID of the bet to claim      Raises:     Error if bet doesn't exist     Error if caller is not the bettor     Error if winnings already claimed     Error if event not resolved     Error if bet did not win

   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  claimWinnings(params?: CallParams<PredictionMarketArgs['obj']['claim_winnings(uint64)void'] | PredictionMarketArgs['tuple']['claim_winnings(uint64)void']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['claim_winnings(uint64)void'] | undefined]>

  /**
   * Calls the get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[] ABI method.
   *
   * Get all bets for a user
   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getUserBets(params?: CallParams<PredictionMarketArgs['obj']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | PredictionMarketArgs['tuple']['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_user_bets(address)(uint64,uint64,address,bool,uint64,bool)[]'] | undefined]>

  /**
   * Calls the get_total_bets(uint64)(uint64,uint64) ABI method.
   *
  * Get total bet counts for an event
  Args:     event_id: ID of the event      Returns:     Tuple of (yes_bets, no_bets)      Raises:     Error if event doesn't exist

   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getTotalBets(params?: CallParams<PredictionMarketArgs['obj']['get_total_bets(uint64)(uint64,uint64)'] | PredictionMarketArgs['tuple']['get_total_bets(uint64)(uint64,uint64)']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_total_bets(uint64)(uint64,uint64)'] | undefined]>

  /**
   * Calls the get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64) ABI method.
   *
  * Get event details
  Args:     event_id: ID of the event      Returns:     Event struct with all event data      Raises:     Error if event doesn't exist

   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getEvent(params?: CallParams<PredictionMarketArgs['obj']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | PredictionMarketArgs['tuple']['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_event(uint64)(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)'] | undefined]>

  /**
   * Calls the get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[] ABI method.
   *
   * Get all events
   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getAllEvents(params?: CallParams<PredictionMarketArgs['obj']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | PredictionMarketArgs['tuple']['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_all_events()(uint64,string,uint64,bool,bool,uint64,uint64,uint64,uint64)[]'] | undefined]>

  /**
   * Calls the get_contract_balance()uint64 ABI method.
   *
   * Get contract/app account balance
   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getContractBalance(params?: CallParams<PredictionMarketArgs['obj']['get_contract_balance()uint64'] | PredictionMarketArgs['tuple']['get_contract_balance()uint64']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_contract_balance()uint64'] | undefined]>

  /**
   * Calls the emergency_withdraw()void ABI method.
   *
  * Emergency withdraw function for admin
  Transfers entire app balance to admin account
  Raises:     Error if caller is not admin

   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  emergencyWithdraw(params?: CallParams<PredictionMarketArgs['obj']['emergency_withdraw()void'] | PredictionMarketArgs['tuple']['emergency_withdraw()void']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['emergency_withdraw()void'] | undefined]>

  /**
   * Calls the get_admin()address ABI method.
   *
   * Get the admin address
   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getAdmin(params?: CallParams<PredictionMarketArgs['obj']['get_admin()address'] | PredictionMarketArgs['tuple']['get_admin()address']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_admin()address'] | undefined]>

  /**
   * Calls the get_event_counter()uint64 ABI method.
   *
   * Get the current event counter
   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getEventCounter(params?: CallParams<PredictionMarketArgs['obj']['get_event_counter()uint64'] | PredictionMarketArgs['tuple']['get_event_counter()uint64']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_event_counter()uint64'] | undefined]>

  /**
   * Calls the get_bet_counter()uint64 ABI method.
   *
   * Get the current bet counter
   *
   * @param args The arguments for the contract call
   * @param params Any additional parameters for the call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  getBetCounter(params?: CallParams<PredictionMarketArgs['obj']['get_bet_counter()uint64'] | PredictionMarketArgs['tuple']['get_bet_counter()uint64']>): PredictionMarketComposer<[...TReturns, PredictionMarketReturns['get_bet_counter()uint64'] | undefined]>

  /**
   * Makes a clear_state call to an existing instance of the PredictionMarket smart contract.
   *
   * @param args The arguments for the bare call
   * @returns The typed transaction composer so you can fluently chain multiple calls or call execute to execute all queued up transactions
   */
  clearState(params?: AppClientBareCallParams): PredictionMarketComposer<[...TReturns, undefined]>

  /**
   * Adds a transaction to the composer
   *
   * @param txn A transaction to add to the transaction group
   * @param signer The optional signer to use when signing this transaction.
   */
  addTransaction(txn: Transaction, signer?: TransactionSigner): PredictionMarketComposer<TReturns>
  /**
   * Returns the underlying AtomicTransactionComposer instance
   */
  composer(): Promise<TransactionComposer>
  /**
   * Simulates the transaction group and returns the result
   */
  simulate(): Promise<PredictionMarketComposerResults<TReturns> & { simulateResponse: SimulateResponse }>
  simulate(options: SkipSignaturesSimulateOptions): Promise<PredictionMarketComposerResults<TReturns> & { simulateResponse: SimulateResponse }>
  simulate(options: RawSimulateOptions): Promise<PredictionMarketComposerResults<TReturns> & { simulateResponse: SimulateResponse }>
  /**
   * Sends the transaction group to the network and returns the results
   */
  send(params?: SendParams): Promise<PredictionMarketComposerResults<TReturns>>
}
export type PredictionMarketComposerResults<TReturns extends [...any[]]> = Expand<SendAtomicTransactionComposerResults & {
  returns: TReturns
}>

